...
接下来仔细看一下具体过程, 我们知道入参 `blocks` 的第一个维度是相互独立的, 因此我们认为 `blocks` 的形状是 `(1, 210)`, 前几个步骤是将这 210 个 uint8 分为 4 块, 我们记作:
...
step1: `ql = ql.reshape((n_blocks, -1, 1, 64)) >> np.array([0, 4], dtype=np.uint8).reshape((1, 1, 2, 1))`
...
step2: `ql = (ql & np.uint8(0x0F)).reshape((n_blocks, -1, 32))`
...
而同样的道理, `qh=[b0, ..., b64]`, 我们将每个数字的最低,次低,次高,高 2 比特位记作 `b0_0`, `b0_1`, `b0_2`, `b0_3`, 最终 `qh` 被转化为:
...
def my_dequant(blocks):
    ...
    a = blocks[:128]
    b = blocks[128:192]
    c = blocks[192:208].astype(np.float32)
