...
先从 AutoModelForCausalLM 入手, 切入点是解释这行代码发生了什么:
...
为此我们先看看 `transformers/models/auto` 相关代码里硬编码写的一些字典映射
...
CONFIG_MAPPING_NAMES = OrderedDict(
  [
    ("gpt2", "GPT2Config"),  # GPT2Config 代表 models/gpt2/configuration_gpt2/GPT2Config
    ("llama", "LlamaConfig"),
  ]
)
...
首先, 类似 `CONFIG_MAPPING_NAMES` 这种字典的键和值有如下类型:

- model-type: 例如 `"deberta-v2"`, 可以是中划线或下划线连接
  - `transformers.models.auto.configuration_auto.MODEL_NAMES_MAPPING` 的键
- model-display-name: 例如 `"Audio Spectrogram Transformer"`, 可以用空格连接, 对应于[文档页面](https://huggingface.co/docs/transformers/model_doc/audio-spectrogram-transformer)上的模型名称
  - `transformers.models.auto.configuration_auto.MODEL_NAMES_MAPPING` 的值
- package-name: 例如 `"llama"`, 对应于目录结构 `src/transformers/llama`
- class-name: 例如 `"LlamaForCausalLM"`, 对应与类名 `src/transformers/llama`

```yaml
# transformers.models.auto.configuration_auto
CONFIG_MAPPING_NAMES:
  key: "model-type:gpt2"
  value: "config-class-name:GPT2Config"
...
from transformers import AutoModelForCausalLM
import transformers
model_type = AutoModelForCausalLM._model_mapping[transformers.models.llama.configuration_llama.LlamaConfig]
model_type is transformers.models.llama.modeling_llama.LlamaForCausalLM  # True
```
