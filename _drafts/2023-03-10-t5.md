...
对于 huggingface transformers 的源码阅读而言，本文主要的关注点在于以下部分，首先 transformers github 项目的目录结构如下（节选）
...
src/transformers
  ...
  - models/  # 每个模型为一个单独的文件夹, 每个文件夹的文件结构比较固定, 参考t5子文件夹
    - t5/
      - __init__.py
      - convert_t5_original_tf_checkpoint_to_pytorch.py  # 有些模型原始官方仓库的权重需要通过转换得到huggingface的格式, 这种情况下会维护一个转换脚本
      ...
      - tokenization_t5_fast.py  # 每个模型的fast版本的tokenizer实现, 速度较快, 依赖于huggingface tokenizers
      - ...
    - ...
...
- huggingface transformers 官方文档对各个模型所使用的 tokenizer 的概述：[官方文档](https://huggingface.co/docs/transformers/main/en/tokenizer_summary)
- BPE 算法详解博客：[博客](https://towardsdatascience.com/byte-pair-encoding-subword-based-tokenization-algorithm-77828a70bee0)

【待确定放在哪】[对比几种不同的tokenizer](https://huggingface.co/learn/nlp-course/chapter6/4?fw=pt)


取决于不同的 tokenizer 实现, huggingface tokenizers 中的 Tokenizer 在encode阶段通常会进行如下几个步骤
...
首先说明一下 huggingface transformers 与 huggingface tokenizers 之间的关系：transformers 4.x 版本中每个模型都会尽量支持两种 tokenizer 的实现, slow版本的实现与fast版本的实现, 后者依赖于 tokenzers 包, 而前者不依赖。transformers 中, 每个模型都会对应于其特有的 tokenizer, 例如: t5 模型的 tokenizer 为 `T5Tokenizer` 和 `T5TokenizerFast`。继承关系如下：
...
源码及官方文档中对特殊token有若干个相似的方法与命名，让人非常困惑，因此需要理清【待后续补充】
...
def _encode_plus(self, text, text_pair):
    ...
    # prepare_for_model 需要做后处理: 首尾加特殊token, 
...
在 huggingface transformers 库中, fast 版本的 tokenizer 实际上利用了 huggingface tokenizers 的一些内容, 因此 `PretrainedTokenizerFast` 是可以训练的，而 slow 版本的 tokenizer 不支持训练。使用方法如下：
...
注意：这种做法适用于与现有的一个 tokenizer 一致的设定, 例如：BOS token等, 绝大多数情况下, 已经足够使用。如果确实需要做比较大的调整，则需要借助 huggingface tokenizers 包。
