...
```python
import torch
N, d_in, d_out = 1000, 32, 64
X = torch.randn(d_in, N)
W = torch.randn(d_out, d_in)
ori_W = W.clone()

Losses = torch.zeros_like(W)  # 每次量化的损失
...
class Quantizer:
    ...
    def get_quantizer_params(self, W):
        ...
        # 即最大值量化后为 num_grids, 最小值量化后为 0, 每一行的 scale 都是不同的
...
# sanity checks
# (1) 量化后的总损失量应该等于每次量化的损失之和
torch.allclose(torch.sum((Q@X - ori_W@X)**2 / N), Losses.sum())

# (2) 量化后的结果都恰好在量化网格上
Q_int, Q1 = quantizer.quantize(Q)
torch.allclose(Q, Q1)

# 直接进行取整舍入量化的损失
W1 = quantizer.quantize(ori_W)[1]
print("direct quantizer:", torch.sum((W1@X - ori_W@X)**2 / N))
# GPTQ 量化的损失, 一般来说都会比直接舍入取整要好
print("GPTQ:", torch.sum((Q@X - ori_W@X)**2 / N))
```
...
对于需要频繁更新整个海塞矩阵的逆, 作者观察到 \eqref{OBC-2} 式的更新公式与 Cholesky 矩阵分解之间的联系, 从而避免了对海塞矩阵逆的更新. 具体来说, 在 Cholesky 矩阵分解过程的迭代过程与海塞矩阵逆的更新过程存在对应关系, 这里将海塞矩阵的逆记作 $H^{-1}$, 从 OBQ 算法的更新公式来看, 海塞矩阵的逆的更新过程记作 $H^{(1)}=H^{-1}, \ldots, H^{(n+1)}$, 注意 $H^{(i)}$ 为 $n-i+1$ 阶方阵, 海塞矩阵的逆的更新过程是:

$$
H^{(i+1)}=[H^{(i)}-\frac{1}{(H^{(i)})_{1,1}}(H^{(i)})_{:,1}(H^{(i)})_{1,:}]_{2:, 2:}
$$

而将 $H^{-1}$ 进行 Cholesky 矩阵分解的过程为, 定义一系列矩阵 $A^{(1)}=H^{-1}, \ldots, A^{(n+1)}=I$.
...
$$
C^{(i+1)} = B^{(i)}-\frac{1}{a_{i,i} }\mathbf{b}_i\mathbf{b}_i^T=[C^{(i)}-\frac{1}{C^{(i)}_{1,1} }C^{(i)}_{:,1}C^{(i)}_{1,:}]_{2:, 2:}
$$

并且注意到 $C^{(1)}=A^{(1)}=H^{(-1)}$, 因此 $C^{(i)}=H^{(i)}$, 且

$$
H^{(i)}_{1,:}=C^{(i)}_{1,:}=[a_{i,i}, \mathbf{b}_i^T]=\sqrt{a_{i,i} }(L^{(i)}_{i:, i})^T=\sqrt{H^{(i)}_{1,1} }\cdot (L^{(i)}_{i:, i})^T=\sqrt{H^{(i)}_{1,1} }\cdot (L_{i:, i})^T
$$



所以权重更新的迭代过程如下, 其中上标 $(q)$ 表示第 $q$ 轮迭代

$$
\begin{align}
\delta\mathbf{w}^{(q)}&=-\frac{w_q^{(q)}-\text{quant}(w_q^{(q)})}{[H^{(q)}]_{1,1}}H^{(q)}_{:,1} \\
&=-\frac{w_q^{(q)}-\text{quant}(w_q^{(q)})}{L_{q,q} }L_{q:,q} 
\end{align}
$$

这样, **我们只需要利用 $H^{-1}$ Cholesky 分解式 $H^{-1}=LL^T$ 中的 $L$ 矩阵, 无须对 $H^{-1}$ 进行显式的更新就可以得到每一步的更新权重**.

结合 Lazy Batch-Updates, 实际上, 我们可以将权重更新式限制在 block 内, 然后再通过一次更新将此 block 之后的全部列进行更新. 这样就起到了节省内存带宽的作用, 从而显著提升算法效率

### GPTQ 算法流程

原文中的算法流程图如下

![](../assets/figures/gptq/GPTQ-algorithm1.png)

这里做几点简要说明:

- 这里的算法流程对权重的每一行是并行处理的
- 算法第 3 行左侧的 $H^{-1}$ 实际上是 $H^{-1}=LL^T$ 中的 $L^T$
- 算法的倒数第 5 行的分子进行平方后除以 2 才代表了本次迭代造成的损失增量
- 算法的倒数第 4 行实际上还更新了被量化的权重项本身, 更新后其权重为 $0$, 而被量化的权重所在列靠前的权重的更新项实际上是 $0$, 因此可以统一这么写
- 算法倒数第 2 行表示对 block 之后的全部权重进行一次性更新, 实际上也就是倒数第 4 行将列的范围 $j:(i+B)$ 扩展到 $i+B:$

这里给出 GPTQ 算法的实现 (不加Lazy Batch-Updates以及Cholesky Reformulation的版本与前面的代码相同)
...
经测试, 在 `block_size=128`, `d_in=d_out=4096`, 进行 GPTQ 4-bit 量化时, 使用单张 V100 显卡实测量化速度, 这三种实现方式的耗时分别为: 21s, 30s, 3s. 由此可以看出 GPTQ 算法中使用 Lazy Batch-Updates 以及 Cholesky Reformulation 的技巧确实能显著降低算法的运行时间.
...
- 选择一个合适的量化函数
- 收集每一个全连接层在校准数据上的输入: 通过 `nn.Module` 的 hook 机制实现
- GPTQ 算法运行: 上一节已经有基础的实现
- 量化后的模型推理过程的实现: 需要编写自定义的 CUDA 核函数实现 (gptq 原始实现), 或者使用 OpenAI triton 实现 (gptq-for-llama, auto-GPTQ 两个 repo 中实现)

## 运行

后续各节的源码解析主要是为解释如下过程的具体细节

```bash
git clone https://github.com/IST-DASLab/gptq.git

# 量化: 作者并未提供保存量化后的llama模型脚本, 但在 opt.py 中有涉及模型保存的代码
python llama.py /path/to/huggingface_llama_7B wikitext2 \
  --percdamp 0.01 \  # default: 0.01
  --wbits 3 \  # default: 16
  --groupsize 128 \  # default: -1
...
这里的参数细节在 [GPTQ-triton](https://github.com/fpgaminer/GPTQ-triton) 也有比较充分的解释, 可以参考

## GPTQ 源码解析: 量化函数的选择

这里只探讨 `nn.Linear` 的量化, 注意原始论文中提及了对卷积层的量化. `nn.Linear` 中包含权重参数 `weight` 与 `bias`, GPTQ 量化算法仅对 `weight` 进行量化, 即量化后模型参数为整数形式的 `weight` (以及 `scales` 和 `zeros` 等) 和浮点数形式的 `bias`. 在对量化后的模型进行推理时, 需要将浮点数形式的输入 `x` 与整数形式的 `weight` 和浮点数形式的 `bias` 进行运算.

备注: huggingface transformers 对 OpenAI 的 GPT-2 模型的一些线性层使用了一维卷积实现, 本质上就是 `nn.Linear`.



## GPTQ 源码解析: 校准数据在待量化层上的输入

## GPTQ 源码解析: GPTQ 算法实现细节

注意, “每一层”在被量化后, 该层的权重会被量化后的权重替换, 下一层的输入是上一层量化后的权重计算的输出. 这里的“一层”指的是
...
关于这一点, 此处还有一个更为细节之处, 对于每个“层”内部, 可以考虑按如下两种量化顺序:

```python
# --true-sequential 为 True 时, 按照llama模型在运行时的次序逐组优化. (gptq中的实现默认值为False, AutoGPTQ的实现里中默认为True)
# 这样量化效果最好, 但会慢一些: 量化第2组参数时, 需要用第1组量化后的参数重新跑一遍
sequential = [
...
# --true-sequential 为 False 时, 这样的量化效果相对差一些, 但量化速度快一些
