...
对于需要频繁更新整个海塞矩阵的逆, 作者观察到 \eqref{OBC-2} 式的更新公式与 Cholesky 矩阵分解之间的联系, 从而避免了对海塞矩阵逆的更新. 具体来说, 在 Cholesky 矩阵分解过程的迭代过程与海塞矩阵逆的更新过程存在对应关系, 这里将海塞矩阵的逆记作 $A$, 从 OBQ 算法的更新公式来看, 海塞矩阵的逆的更新过程记作 $A^{(1)}=A, \ldots, A^{(n+1)}$, 注意 $A^{(i)}$ 均为 $n$ 阶方阵, 海塞矩阵的逆的更新过程是取 $(A^{(i)})_{i:,i:}, i=1,\ldots,n$.

$$
A^{(i+1)}=[A^{(i)}-\frac{1}{(A^{(i)})_{i,i}}(A^{(i)})_{i,:}(A^{(i)})_{:,i}]
$$

而将 $A$ 进行 Cholesky 矩阵分解的过程中, 迭代更新过程记作 $A^{(1)}=A, \ldots, A^{(n+1)}$, 而

$$
A^{(i)} = L^{(i)}A^{(i+1)}(L^{(i)})^T \\
(A^{(i+1)})_{i+1:,i+1:}=(A^{(i)})_{i+1:,i+1:}-\frac{1}{(A^{(i)})_{i,i}}(A^{(i)})_{i+1,:}(A^{(i)})_{:,i+1}
$$

因此我们可以看出: 海塞矩阵的更新过程等价于 Cholesky 矩阵分解的迭代过程:

$$
\begin{align*}
H^{(1)}&=A^{(1)} \\
H^{(2)}&=[L^{(1)}A^{(2)}(L^{(1)})^T]_{2:, 2:} \\
H^{(3)}&=[L^{(1)}L^{(2)}A^{(3)}(L^{(2)})^T(L^{(1)})^T]_{3:, 3:}
\end{align*}
$$
...
$$
\delta\mathbf{w}^{(q)}=-\frac{w_q^{(q)}-\text{quant}(w_q^{(q)})}{[H^{-1}]_{qq}^{(q)}}(\mathbf{H}^{-1})_{:,q}^{(q)}
$$

的更新过程实际上是:

$$
\delta\mathbf{w}^{(q)}=-\frac{w_q^{(q)}-\text{quant}(w_q^{(q)})}{(L^{(q)})^2}(L^{(q)})_{q+1:,q}
$$
...
$$
\begin{align*}
\delta\mathbf{w}^{(1)}=-\frac{w_1^{(1)}-\text{quant}(w_1^{(1)})}{L_{1,1}^2}L_{2:,1}\\
\delta\mathbf{w}^{(2)}=-\frac{w_2^{(2)}-\text{quant}(w_2^{(2)})}{L_{2,2}^2}L_{3:,2}
\end{align*}
$$
...
这里做几点简要说明【似乎还有些迷糊，待确认】:

- 这里的算法流程对权重的每一行是并行处理的
- 算法第 3 行左侧的 $H^{-1}$ 实际上是 $H^{-1}=LL^T$ 中的 $L^T$, 并且对 $L^T$ 的对角线元素进行了平方处理【待确认】
- 算法的倒数第 5 行的分子进行平方后才代表了此次量化引起的损失增加量, 图中写的有问题【待确认】
...
**总的来说, 原文中算法流程图有一定的误导性**, 这里给出 GPTQ 算法的实现(不加Lazy Batch-Updates以及Cholesky Reformulation的版本与前面的代码相同)
