...
class Session(BaseModel):
    ...
    model_config = ConfigDict(
        extra='forbid',  # 不允许实例化时传入额外字段, 默认情况是允许的
        arbitrary_types_allowed=True,  # 允许注解使用没有继承自 BaseModel 的类, 例如这里的 Event. 默认情况不允许
        alias_generator=alias_generators.to_camel,  # 允许传入驼峰命名的变量, 例如使用 appName
        populate_by_name=True,  # 允许使用 app_name 来构造实例, 配合 alias_generator 使用
    )
...
s = Session(appName="abc", id="test", events=[Event()])  # ok
s = Session(app_name="abc", id="test", events=[Event()])  # ok
s.appName  # error, 使用时必须严格按照类定义来
s = Session(app_name="abc", id="test", events=[Event()], extra_info="xx")  # error
```
